...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < max_retries; ++i)
	{
					...
					sizeof(server_addr)) == 0)
		{
			...
			std::string request = "CONNECT 8000\nGET / HTTP/1.1\r\nHost: deno\r\nConnection: close\r\n\r\n";
			...
			close(sock);
			// Check for firecracker CONNECT response line "OK <number>\n"
			if (received < 3 || buffer[0] != 'O' || buffer[1] != 'K') {
				return 1;
			}
			char* response = (char *)memchr(buffer, '\n', received);
			if (response == NULL) {
				return 2;
			}
			response++;
			if ((response - buffer + 8) >= received || memcmp(response, "HTTP/1.1", 8) != 0) {
